...
async function ensureApplicationIdIsDefinedForManagedProjectAsync({ graphqlClient, projectDir, projectId, exp, vcsClient, nonInteractive, autoSelectDefault = false, }) {
...
async function configureApplicationIdAsync({ graphqlClient, projectDir, projectId, exp, nonInteractive, autoSelectDefault = false, }) {
    if (nonInteractive) {
    ...
    if (autoSelectDefault && suggestedAndroidApplicationId) {
        ...
        log_1.default.withTick(`Set "android.package" to ${chalk_1.default.bold(packageName)} (modified app.json)`);
    }
    else {
        ...
        ({ packageName } = await (0, prompts_1.promptAsync)({
            name: 'packageName',
            type: 'text',
