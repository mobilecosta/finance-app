...
async function ensureBundleIdentifierIsDefinedForManagedProjectAsync({ graphqlClient, projectDir, projectId, exp, vcsClient, nonInteractive, autoSelectDefault = false, }) {
...
async function configureBundleIdentifierAsync({ graphqlClient, projectDir, projectId, exp, nonInteractive, autoSelectDefault = false, }) {
    if (nonInteractive) {
    ...
    if (autoSelectDefault && suggestedBundleIdentifier) {
        ...
        log_1.default.withTick(`Set "ios.bundleIdentifier" to ${chalk_1.default.bold(bundleIdentifier)} (modified app.json)`);
    }
    else {
        ...
        ({ bundleIdentifier } = await (0, prompts_1.promptAsync)({
