...
exports.collectAggregateStepErrors = collectAggregateStepErrors;
...
class BuildWorkflowValidator {
    ...
    async validateAsync() {
        ...
        const errors = [];
        errors.push(...collectAggregateStepErrors(orderedView, {
 ...
 */
function collectAggregateStepErrors(steps, { runtimePlatform }) {
    return [
...
function validateAllowedPlatforms(steps, runtimePlatform) {
    const errors = [];
    for (const step of steps) {
        if (!step.canBeRunOnRuntimePlatform()) {
            const error = new errors_1.BuildConfigError(`Step "${step.displayName}" is not allowed on platform "${runtimePlatform}". Allowed platforms for this step are: ${(0, nullthrows_1.nullthrows)(step.supportedRuntimePlatforms, `step.supportedRuntimePlatforms can't be falsy if canBeRunOnRuntimePlatform() is false`)
                .map(p => `"${p}"`)
                .join(', ')}.`);
            errors.push(error);
        }
