...
function logConditionEvaluationError(logger, err, subject, ifCondition) {
    logger.error({ err });
    logger.error(`Runner failed to evaluate if it should execute ${subject}, using its if condition "${ifCondition}". This can be caused by trying to access non-existing object property. If you think this is a bug report it here: https://github.com/expo/eas-cli/issues.`);
}
